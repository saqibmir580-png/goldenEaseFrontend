...
const FaceVerify: React.FC = () => {
  ...
  const tabs = [
    ...
    { id: "marriage_certificate", label: "Marriage Certificate", icon: <MdOutlineChurch size={18} /> },
    { id: "emirates_id", label: "Emirates ID", icon: <House size={18} /> },
    { id: "property_document", label: "Property Ownership", icon: <House size={18} /> },
  ];
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userData.image_url) {
          const img = new Image();
          img.crossOrigin = "anonymous";
          img.src = userData.image_url.startsWith('http')
            ? userData.image_url
            : `http://localhost:8000${userData.image_url}`;
          img.onload = () => {
            setUploadedImage(img);
            setLoading(false);
          };
        ...
        // Get verification status from navigation or API
        if (location.state?.verificationStatus) {
          setVerificationStatus(location.state.verificationStatus);
        } else {
          const response = await fetch(`http://localhost:8000/admin/validate/${id}`);
        ...
        console.error("Error loading user data or verification status:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getDocumentByType = (type: string): VerificationDetail | null => {
    return verificationStatus?.results.find(doc => doc.document_type === type) || null;
  };
  ...
  const getStatusIcon = (status: string, isValid: boolean) => {
    if (isValid && status === 'valid') {
      ...
      return <ImCross className="text-red-600" />;
    }
  };
  ...
  const handleTabClick = async (tabId: string) => {
    ...
    if (tabId !== 'photo-verification' && !verificationStatus) {
      try {
        setIsValidating(true);
        const response = await fetch(`http://localhost:8000/admin/validate/${id}`);
        if (!response.ok) {
  ...
  const renderPhotoVerification = () => (
    <div className="space-y-6 min-h-screen w-full">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {/* Right Side: Validation Criteria */}
        <div className="flex-1">
          <div className="w-full max-w-md border p-4 rounded-lg bg-white">
            <h3 className="text-lg font-semibold mb-2 text-gray-800">Validation Criteria</h3>
            {modelsLoaded ? (
  ...
  const renderDocument = (docType: string, title: string) => {
    ...
    const description = `Required for visa processing. View ${title.toLowerCase()} document information and validity status.`;
          ...
          <div className="text-center bg-gray-50 rounded-lg p-12">
            <div className="text-gray-400 mb-4">
              {docType === 'passport' ? (
                ...
                <MdOutlineChurch className="mx-auto text-6xl mb-4" />
              ) : (
                <House className="mx-auto text-6xl mb-4" />
              )}
            </div>
            <h3 className="text-xl font-semibold text-gray-600 mb-2">Missing {title}</h3>
            <p className="text-gray-500">This document is required for visa processing. Please upload or verify this document.</p>
    ...
    const fileUrl = document.file_path ? `http://localhost:8000/${document.file_path.replace(/\\/g, '/')}` : '';
    const status = document.is_valid
      ? isExpiringSoon(document.details?.expiry_date) ? 'pending_renewal' : 'valid'
      : document.details?.expiry_date && new Date(document.details.expiry_date) < new Date() ? 'expired' : 'invalid';
            ...
            <div className="bg-white border-2 border-gray-200 rounded-lg overflow-hidden">
              ...
              <div className="p-4 bg-gray-50 flex gap-2 justify-center">
                ...
                <a
                  href={fileUrl}
                  download
                  className={`bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors flex items-center gap-2 ${!fileUrl ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <FaDownload /> Download
                </a>
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white border rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  {getStatusIcon(status, document.is_valid)}
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(status)}`}>
                    {status.replace('_', ' ').toUpperCase()}
                  </span>
                </div>
              </div>
              {isExpiringSoon(document.details?.expiry_date) && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                  ...
                  <p className="text-yellow-700 text-sm mt-1">
                    This document will expire within 6 months. Please renew it soon to ensure visa eligibility.
                  </p>
                </div>
              )}
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-600">Document Number:</span>
                  <span className="text-gray-800 font-mono">{document.details?.document_number || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Holder Name:</span>
                  <span className="text-gray-800">{document.details?.holder_name || user?.name || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Issued Date:</span>
                  <span className="text-gray-800">{document.details?.issued_date ? new Date(document.details.issued_date).toLocaleDateString() : 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Expiry Date:</span>
                  <span className={`${document.details?.expiry_date && new Date(document.details.expiry_date) < new Date() ? 'text-red-600 font-medium' : 'text-gray-800'}`}>
                    {document.details?.expiry_date ? new Date(document.details.expiry_date).toLocaleDateString() : 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Issuer:</span>
                  <span className="text-gray-800">{document.details?.issuer || 'N/A'}</span>
                </div>
              </div>
            </div>
            {document.details && (
                ...
                <div className="space-y-2">
                  {docType === 'property_document' ? (
                    <>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Property Type:</span>
                        <span className="text-gray-800">{document.details.property_type || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Property Address:</span>
                        <span className="text-gray-800">{document.details.property_address || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Property Value:</span>
                        <span className="text-gray-800">{document.details.property_value || 'N/A'}</span>
                      </div>
                    </>
                  ) : (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Nationality:</span>
                      <span className="text-gray-800">{document.details.nationality || 'N/A'}</span>
                    </div>
                  )}
                  {document.message && (
                    <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                      <p className="text-sm text-blue-800">
                        <strong>Validation Message:</strong> {document.message}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className={`border rounded-lg p-6 ${
              document.is_valid && status === 'valid'
                ? 'bg-green-50 border-green-200'
                : 'bg-red-50 border-red-200'
            }`}>
              <h3 className="text-lg font-semibold mb-2">Validity Summary</h3>
              <p className={`${
                document.is_valid && status === 'valid'
                  ? 'text-green-700'
                  : 'text-red-700'
                ...
                {document.is_valid && status === 'valid'
                  ? '✅ This document is valid and can be used for visa processing.'
                  : status === 'expired'
                  ? '❌ This document has expired and needs to be renewed for visa processing.'
                  : status === 'pending_renewal'
                  ? '⚠️ This document is valid but renewal is recommended for visa processing.'
                  : '❌ This document is invalid and cannot be used for visa processing.'
                }
              </p>
              {document.message && (
                <p className="text-red-600 text-sm mt-2">
                  <strong>Issue:</strong> {document.message}
                </p>
              )}
            </div>
          </div>
...
const renderVerificationOverview = () => (
  <div className="max-w-7xl mx-auto min-h-screen flex flex-col px-4">
    <div className="flex flex-col lg:flex-row gap-6 p-6">
      ...
      <div className="lg:w-1/2 flex-1 bg-white border rounded-lg p-6 h-[400px] overflow-y-auto">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">User Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <p className="text-gray-600 font-medium">Full Name</p>
      ...
      {/* Right Side: Document Status Summary */}
      <div className="lg:w-1/2 flex-1 bg-white border rounded-lg p-6 h-[400px] overflow-y-auto">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">Document Status Summary</h3>
        {loading ? (
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {tabs
                .filter(tab => tab.id !== 'photo-verification')
                .map(({ id, label }) => {
                  const document = getDocumentByType(id);
                  ...
                  return (
                    <div key={id} className="flex items-center justify-between border-b pb-2">
                      ...
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        document ? getStatusColor(status) : 'bg-gray-100 text-gray-800'
                      }`}>
                        {document ? status.replace('_', ' ').toUpperCase() : 'Required for Visa'}
                      </span>
                    </div>
                  );
                })}
              <div className="mt-6">
                <p className="text-gray-600">
                  Click the Verify button to view detailed document information.
                </p>
              </div>
            </div>
          ) : (
    ...
    {/* Verify Button at Bottom Center */}
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100">
      <div className="w-full max-w-8xl mx-auto bg-white rounded-2xl shadow-xl p-8">
        ...
        {showTabs ? (
          <div className="flex flex-col gap-8">
            <nav className="bg-white rounded-xl shadow-sm p-4 flex flex-wrap justify-center gap-3">
              {tabs.map((tab) => {
                const document = getDocumentByType(tab.id);
                const hasDocument = document !== null;
                const isValid = document?.is_valid && !isExpiringSoon(document?.details?.expiry_date);

                return (
                  <button
                    key={tab.id}
                    onClick={() => handleTabClick(tab.id)}
                    className={`flex items-center gap-3 px-5 py-3 rounded-lg text-sm font-semibold transition-all duration-300 relative
                      ${
                        activeTab === tab.id
                          ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-md'
                          : 'bg-gray-100 text-gray-700 hover:bg-blue-50 hover:text-blue-700 hover:shadow-sm transform hover:scale-105'
                      }`}
                  >
                    <span className="text-lg">{tab.icon}</span>
                    <span>{tab.label}</span>
                    {tab.id !== 'photo-verification' && (
                      <span className="flex items-center">
                        {hasDocument ? (
                          isValid ? (
                            <span className="w-4 h-4 bg-green-500 rounded-full shadow-[0_0_6px_rgba(34,197,94,0.5)]"></span>
                          ) : (
                            <span className="w-4 h-4 bg-red-500 rounded-full shadow-[0_0_6px_rgba(239,68,68,0.5)]"></span>
                          )
                        ) : (
                          <span className="w-4 h-4 bg-gray-400 rounded-full shadow-[0_0_6px_rgba(107,114,128,0.5)]"></span>
                        )}
                      </span>
                    )}
                  </button>
            ...
            <div className="min-h-96">
              ...
              {activeTab === "emirates_id" && renderDocument("emirates_id", "Emirates ID")}
              {activeTab === "marriage_certificate" && renderDocument("marriage_certificate", "Marriage Certificate")}
              {activeTab === "property_document" && renderDocument("property_document", "Property Ownership")}
            </div>
          </div>
        ) : (
          renderVerificationOverview()
